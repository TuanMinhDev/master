...
export const useSerMSTPartTypeDataSource = () => {
  const { t } = useI18n("Error");
  const api = useClientgateApi();
  const showError = useSetAtom(showErrorAtom);
  const setLoading = useSetAtom(loadPanelAtom);

  const getListSerMSTPartType = async () => {
    setLoading(true);
  ...
  return {
    getListSerMSTPartType,
  };
};

export const useSerMSTPartGroupDataSource = () => {
  const { t } = useI18n("Error");
  const api = useClientgateApi();
  const showError = useSetAtom(showErrorAtom);
  const setLoading = useSetAtom(loadPanelAtom);
